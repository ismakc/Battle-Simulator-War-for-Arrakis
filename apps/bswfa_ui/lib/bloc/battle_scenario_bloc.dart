...
class BattleScenarioBloc
    extends Bloc<BattleScenarioEvent, BattleScenarioState> {
  ...
  FutureOr<void> _onUpdateAttackingLegion(
    _UpdateAttackingLegion event,
    Emitter<BattleScenarioState> emit,
  ) {
    final BattleScenario updatedBattleScenario = state.battleScenario.copyWith(
      attackingLegion: event.attackingLegion,
    );
    emit(BattleScenarioState(battleScenario: updatedBattleScenario));
  }

  FutureOr<void> _onUpdateDefendingLegion(
    _UpdateDefendingLegion event,
    Emitter<BattleScenarioState> emit,
  ) {
    final BattleScenario updatedBattleScenario = state.battleScenario.copyWith(
      defendingLegion: event.defendingLegion,
    );
    emit(BattleScenarioState(battleScenario: updatedBattleScenario));
  }
...
class BattleScenarioEvent with _$BattleScenarioEvent {
  const factory BattleScenarioEvent.updateAttackingLegion(
    AttackingLegion attackingLegion,
  ) = _UpdateAttackingLegion;

  const factory BattleScenarioEvent.updateDefendingLegion(
    DefendingLegion defendingLegion,
  ) = _UpdateDefendingLegion;
...
abstract class BattleScenarioState with _$BattleScenarioState {
  const factory BattleScenarioState({required BattleScenario battleScenario}) =
